      ...
      <v-card-text>
        ...
        <div v-if="addCragBtn">
          <p class="text-right mt-3">
            ...
            <v-btn
              :to="`/a/crags/new?redirect_to=${$route.fullPath}&name=${query}`"
              text
              small
            >
              <v-icon left>
...
export default {
  ...
  computed: {
    cragRouteRedirectUrl () {
      ...
      return `/a/crags/${this.crag.id}/${this.crag.slug_name}/routes/new?redirect_to=${this.$route.path}&added_from_ascent=true&${routeParams.join('&')}`
    }
  },
